...
// 모바일에서 스크롤 시 주소창이 사라지면서 발생하는 resize 이벤트를 구분하기 위해 사용
...
// [MODIFIED] Spline 오브젝트의 반응형 속성을 반환하는 함수 수정
function getResponsiveSplineProperties() {
    const isMobile = window.innerWidth <= 768;
    // 모바일일 경우, 크기는 1.8배로 키우고 x, y축 위치를 조정하여 중앙에 오도록 합니다.
    if (isMobile) {
        return {
            scaleMultiplier: 1.4,
            positionX: 400, // X축 위치를 0으로 설정하여 수평 중앙 정렬
    ...
    return {
        scaleMultiplier: 1.0,
        positionX: 300, // 데스크탑에서는 원래 X 위치 사용
        positionYOffset: 0
    };
}
            ...
            heroTextAnim.eventCallback("onComplete", () => {
                ...
                if (splineObjectContainer && winhubObject && !splineIntroPlayed) {
                    ...
                    const targetPosition = {
                        x: responsiveProps.positionX !== null ? responsiveProps.positionX : originalWinhubState.position.x,
...
// [MODIFIED] 리사이즈 시 Spline 오브젝트가 부드럽게 변하도록 수정
function handleSubPageResize() {
    if (typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') return;
   
    // 모바일에서 스크롤 시 주소창 높이 변화로 인한 리사이즈 이벤트 방지
    const isRealResize = window.innerWidth !== cachedWindowWidth;
    if (!isRealResize) {
        return; 
    }
    ...
    // Spline 오브젝트의 위치와 크기를 즉시 부드럽게 애니메이션 처리
    const splineObjectContainer = document.getElementById("threejs-object-container");
    const winhubObject = subpageSplineApp ? subpageSplineApp.findObjectByName("Winhub") : null;
    ...
    if (winhubObject && originalWinhubState) {
        ...
        // 기존 애니메이션을 덮어쓰면서 부드럽게 크기와 위치 변경
        gsap.to(winhubObject.scale, {
            ...targetScale,
            duration: 0.5,
            ease: "power2.out",
            overwrite: true
        });
        gsap.to(winhubObject.position, {
            ...targetPosition,
            duration: 0.5,
            ease: "power2.out",
            overwrite: true
    ...
    gsap.delayedCall(0.5, () => {
        ...
        if (subHeroContentOnResize) {
            ...
            animateHeroText(); // 텍스트 애니메이션만 독립적으로 실행
        }
        
        // 애니메이션되지 않는 최종 상태(회전 등) 설정
        if (splineObjectContainer && splineIntroPlayed && winhubObject) {
            gsap.set(splineObjectContainer, { autoAlpha: 1 });
            winhubObject.visible = true;
